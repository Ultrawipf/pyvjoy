...
class _FFB_EFF_PERIOD(Structure):
	_pack_ = 1
	_fields_ = [
		('EffectBlockIndex',c_uint32),
		('Magnitude',c_uint32),
...
class _FFB_EFF_ENVLP(Structure):
	_pack_ = 1
	_fields_ = [
		('EffectBlockIndex',c_uint32),
...
class _FFB_EFF_CONST(Structure):
	_pack_ = 1
	_fields_ = [
		('EffectBlockIndex',c_uint32),
		('Magnitude',c_int16),
...
class FFBCallback():
	...
	def __init__(self):
		...
		def ffbCallback(ffbpacket,userdata):
			...
			parsedData = FFBCallback._ffbPacketToDict(ffbpacket)
			if (parsedData["dev"] in self.callbacks):
				self.callbacks[parsedData["dev"]](parsedData)
	...
	@staticmethod
	def _ffbPacketToDict(ffbpacket : _FFB_DATA):
		"""Helper function to store parsed ffb data in a python dict"""
		t = c_int(0)
